...
class NetworkWithJit():
    ...
    def feedforward(self, A):
        for idx, layer in enumerate(self.nn_architecture):
            ...
            W = self.weights[idx]
            b = self.biases[idx]
            A = single_layer_forward_propagation_jit(A, W, b, activ_function)
        return A
...
#@jit(nopython=True)
def single_layer_forward_propagation_jit(A, W, b, activation="relu"):
    
    if activation == "relu":
        activation_func = relu_jit
    elif activation == "sigmoid":
        activation_func = sigmoid_jit
    else:
        raise Exception('Non-supported activation function')
...
if __name__ == '__main__':
    ...
    n2 = NetworkWithJit(nn_architecture)
